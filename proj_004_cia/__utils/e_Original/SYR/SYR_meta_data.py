# ────────────────────────────────────────────────────────────────────────────────────────────
# Enhanced Country Meta Data - SYR
# ────────────────────────────────────────────────────────────────────────────────────────────
...
# ENHANCED COUNTRY META DATA FOR SYR
# ────────────────────────────────────────────────────────────────────────────────────────────
# This dictionary contains comprehensive enhanced metadata for SYR.
# Includes base country information plus recent indicator data for all topics.
# 
# Usage:
#   from impact_titan.impact_3_data.h_saved_world_system.countries.a_impact_country_meta.Original.SYR.SYR_meta_data import SYR_meta_data
#   
#   country_name = SYR_meta_data['name']
#   gdp_data = SYR_meta_data.get('wb_gdp_capita', {})
#   population = SYR_meta_data.get('wb_total_pop', {})
# ────────────────────────────────────────────────────────────────────────────────────────────
...
SYR_meta_data = {
    ...
    "ciLongitude": 38,
    "ciCapital": "Damascus",
    "capitalLatitude": 33.51307,
    "capitalLongitude": 36.309581,
    "ciBorders": [
        "IRQ",
        "ISR",
        "JOR",
    ...
    "_retrieval_metadata": {
        ...
        "retrieval_timestamp": "2025-08-08T18:20:32.866248",
        "topic_update_mode": "all",
        "topics_requested": 0,
        "topics_updated": 0,
        "topics_failed": 0,
        ...
        "is_country": True,
        "is_region": False,
        "enhancement_timestamp": "2025-08-08T18:20:32.870253"
    }
}
...
def get_country_name():
    ...
    return SYR_meta_data.get('name', 'Unknown')
...
def get_topic_data(topic_key):
    ...
    return SYR_meta_data.get(topic_key, {})

def get_all_topics():
    """Get a list of all available topic keys in this dataset."""
    return [key for key in SYR_meta_data.keys() if key.startswith(('wb_', 'custom_', 'country_', 'region_'))]
...
def validate_data_completeness():
    ...
    total_fields = len(SYR_meta_data)
    topics = get_all_topics()
    retrieval_meta = SYR_meta_data.get('_retrieval_metadata', {})
...
if __name__ == '__main__':
    print("═" * 80)
    print(f"ENHANCED COUNTRY META DATA - SYR - VALIDATION")
    print("═" * 80)

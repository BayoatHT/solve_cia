# ────────────────────────────────────────────────────────────────────────────────────────────
# Enhanced Country Meta Data - CYM
# ────────────────────────────────────────────────────────────────────────────────────────────
...
# ENHANCED COUNTRY META DATA FOR CYM
# ────────────────────────────────────────────────────────────────────────────────────────────
# This dictionary contains comprehensive enhanced metadata for CYM.
# Includes base country information plus recent indicator data for all topics.
# 
# Usage:
#   from impact_titan.impact_3_data.h_saved_world_system.countries.a_impact_country_meta.Original.CYM.CYM_meta_data import CYM_meta_data
#   
#   country_name = CYM_meta_data['name']
#   gdp_data = CYM_meta_data.get('wb_gdp_capita', {})
#   population = CYM_meta_data.get('wb_total_pop', {})
# ────────────────────────────────────────────────────────────────────────────────────────────
...
CYM_meta_data = {
    ...
    "ciLatitude": 19.5,
    "ciLongitude": -80.5,
    "ciCapital": "George Town",
    "capitalLatitude": 5.414568,
    "capitalLongitude": 100.329804,
    "ciBorders": [],
    "provinces": [
        "Creek",
        "Eastern",
    ...
    "_retrieval_metadata": {
        ...
        "retrieval_timestamp": "2025-08-08T18:20:29.061438",
        "topic_update_mode": "all",
        "topics_requested": 0,
        "topics_updated": 0,
        "topics_failed": 0,
        ...
        "is_country": True,
        "is_region": False,
        "enhancement_timestamp": "2025-08-08T18:20:29.065439"
    }
}
...
def get_country_name():
    ...
    return CYM_meta_data.get('name', 'Unknown')
...
def get_topic_data(topic_key):
    ...
    return CYM_meta_data.get(topic_key, {})

def get_all_topics():
    """Get a list of all available topic keys in this dataset."""
    return [key for key in CYM_meta_data.keys() if key.startswith(('wb_', 'custom_', 'country_', 'region_'))]
...
def validate_data_completeness():
    ...
    total_fields = len(CYM_meta_data)
    topics = get_all_topics()
    retrieval_meta = CYM_meta_data.get('_retrieval_metadata', {})
...
if __name__ == '__main__':
    print("═" * 80)
    print(f"ENHANCED COUNTRY META DATA - CYM - VALIDATION")
    print("═" * 80)

# ────────────────────────────────────────────────────────────────────────────────────────────
# Enhanced Country Meta Data - AND
# ────────────────────────────────────────────────────────────────────────────────────────────
...
# ENHANCED COUNTRY META DATA FOR AND
# ────────────────────────────────────────────────────────────────────────────────────────────
# This dictionary contains comprehensive enhanced metadata for AND.
# Includes base country information plus recent indicator data for all topics.
# 
# Usage:
#   from impact_titan.impact_3_data.h_saved_world_system.countries.a_impact_country_meta.Original.AND.AND_meta_data import AND_meta_data
#   
#   country_name = AND_meta_data['name']
#   gdp_data = AND_meta_data.get('wb_gdp_capita', {})
#   population = AND_meta_data.get('wb_total_pop', {})
# ────────────────────────────────────────────────────────────────────────────────────────────
...
AND_meta_data = {
    ...
    "subregion": "",
    "_retrieval_metadata": {
        "default_values_applied": 25,
        "fields_already_populated": 19,
        "extracted_fields_total": 44,
        "data_completeness_percentage": 43.18,
        ...
        "retrieval_timestamp": "2025-08-08T18:20:27.880653",
        "topic_update_mode": "all",
        "topics_requested": 0,
        "topics_updated": 0,
        "topics_failed": 0,
        ...
        "is_country": True,
        "is_region": False,
        "enhancement_timestamp": "2025-08-08T18:20:27.884658"
    }
}
...
def get_country_name():
    ...
    return AND_meta_data.get('name', 'Unknown')
...
def get_topic_data(topic_key):
    ...
    return AND_meta_data.get(topic_key, {})

def get_all_topics():
    """Get a list of all available topic keys in this dataset."""
    return [key for key in AND_meta_data.keys() if key.startswith(('wb_', 'custom_', 'country_', 'region_'))]
...
def validate_data_completeness():
    ...
    total_fields = len(AND_meta_data)
    topics = get_all_topics()
    retrieval_meta = AND_meta_data.get('_retrieval_metadata', {})
...
if __name__ == '__main__':
    print("═" * 80)
    print(f"ENHANCED COUNTRY META DATA - AND - VALIDATION")
    print("═" * 80)

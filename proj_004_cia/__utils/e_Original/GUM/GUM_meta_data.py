# ────────────────────────────────────────────────────────────────────────────────────────────
# Enhanced Country Meta Data - GUM
# ────────────────────────────────────────────────────────────────────────────────────────────
...
# ENHANCED COUNTRY META DATA FOR GUM
# ────────────────────────────────────────────────────────────────────────────────────────────
# This dictionary contains comprehensive enhanced metadata for GUM.
# Includes base country information plus recent indicator data for all topics.
# 
# Usage:
#   from impact_titan.impact_3_data.h_saved_world_system.countries.a_impact_country_meta.Original.GUM.GUM_meta_data import GUM_meta_data
#   
#   country_name = GUM_meta_data['name']
#   gdp_data = GUM_meta_data.get('wb_gdp_capita', {})
#   population = GUM_meta_data.get('wb_total_pop', {})
# ────────────────────────────────────────────────────────────────────────────────────────────
...
GUM_meta_data = {
    ...
    "ciLongitude": 144.78333333,
    "ciCapital": "Hagåtña",
    "capitalLatitude": 13.472745,
    "capitalLongitude": 144.752018,
    "ciBorders": [],
    "provinces": [
        "Guam"
    ],
    ...
    "_retrieval_metadata": {
        ...
        "retrieval_timestamp": "2025-08-08T18:20:30.013702",
        "topic_update_mode": "all",
        "topics_requested": 0,
        "topics_updated": 0,
        "topics_failed": 0,
        ...
        "is_country": True,
        "is_region": False,
        "enhancement_timestamp": "2025-08-08T18:20:30.018969"
    }
}
...
def get_country_name():
    ...
    return GUM_meta_data.get('name', 'Unknown')
...
def get_topic_data(topic_key):
    ...
    return GUM_meta_data.get(topic_key, {})

def get_all_topics():
    """Get a list of all available topic keys in this dataset."""
    return [key for key in GUM_meta_data.keys() if key.startswith(('wb_', 'custom_', 'country_', 'region_'))]
...
def validate_data_completeness():
    ...
    total_fields = len(GUM_meta_data)
    topics = get_all_topics()
    retrieval_meta = GUM_meta_data.get('_retrieval_metadata', {})
...
if __name__ == '__main__':
    print("═" * 80)
    print(f"ENHANCED COUNTRY META DATA - GUM - VALIDATION")
    print("═" * 80)

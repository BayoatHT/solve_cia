...
def parse_broadband_media(iso3Code: str, return_original: bool = False)-> dict:
    """Parse Broadcast media from CIA Communications section for a given country."""
    result = {}
    try:
    ...
    pass_data = comms_section.get('Broadcast media', {})
        ...
        if 'note' in pass_data:
            ...
            if isinstance(note_data, dict) and 'text' in note_data:
                ...
                if note and isinstance(note, str) and note.strip():
                    result['broadcast_media_note'] = clean_text(note)
            elif isinstance(note_data, str) and note_data.strip():
                result['broadcast_media_note'] = clean_text(note_data)
    except Exception as e:
        logger.error(f"Error parsing parse_broadband_media for {iso3Code}: {e}")

    return result

if __name__ == "__main__":
    print("="*60)
    print("Testing parse_broadband_media")
    print("="*60)
    for iso3 in ['USA', 'CHN', 'GBR', 'DEU', 'FRA', 'WLD']:
        print(f"\n{iso3}:")
        try:
            result = parse_broadband_media(iso3)
            if result:

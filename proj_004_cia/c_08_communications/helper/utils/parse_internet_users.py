...
def parse_internet_users(iso3Code: str) -> dict:
    ...
    try:
        field_mappings = {'total': 'internet_users_total', 'percent of population': 'internet_users_pct'}
        ...
        logger.error(f"Error parsing parse_internet_users for {iso3Code}: {e}")

    return result

if __name__ == "__main__":
    print("="*60)
    print("Testing parse_internet_users")
    print("="*60)
    for iso3 in ['CHN', 'IND', 'USA', 'IDN', 'BRA', 'WLD']:
        print(f"\n{iso3}:")
        try:
            result = parse_internet_users(iso3)
            if result:

...
def parse_public_debt(pass_data: dict, iso3Code: str = None) -> dict:
    """Parse public debt from CIA Economy section."""
    result = {}
    ...
    try:
        ...
        if yearly_data:
            ...
            result['public_debt_latest'] = yearly_data[0]['value']
            result['public_debt_latest_year'] = yearly_data[0]['year']
        if 'text' in pass_data:
            result['public_debt'] = clean_text(pass_data['text'])
        if 'note' in pass_data:
            note = pass_data['note']
            if isinstance(note, str) and note.strip():
                result['public_debt_note'] = clean_text(note)

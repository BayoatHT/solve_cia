...
def parse_labor_force(pass_data: dict, iso3Code: str = None) -> dict:
    """Parse labor force from CIA Economy section."""
    result = {}
    ...
    try:
        ...
        if yearly_data:
            ...
            result['labor_force_data'] = yearly_data
            result['labor_force_latest'] = yearly_data[0]['value']
            result['labor_force_latest_year'] = yearly_data[0]['year']
        if 'text' in pass_data:
            result['labor_force'] = clean_text(pass_data['text'])
        if 'note' in pass_data:
            note = pass_data['note']
            if isinstance(note, str) and note.strip():
                result['labor_force_note'] = clean_text(note)
    except Exception as e:
        logging.error(f"Error parsing labor_force: {e}")
    return result

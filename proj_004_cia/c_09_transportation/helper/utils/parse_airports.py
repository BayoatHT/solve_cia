...
def parse_airports(iso3Code: str, return_original: bool = False)-> dict:
    """Parse Airports from CIA Transportation section for a given country."""
    result = {}
    try:
    ...
    pass_data = transport_section.get('Airports', {})
    ...
    try:
        ...
        if isinstance(pass_data, dict) and 'note' in pass_data:
            ...
            if note and isinstance(note, str) and note.strip():
                result['airports_total_note'] = clean_text(note)

    except Exception as e:
        logger.error(f"Error parsing parse_airports for {iso3Code}: {e}")

    return result

if __name__ == "__main__":
    print("="*60)
    print("Testing parse_airports")
    print("="*60)
    for iso3 in ['USA', 'BRA', 'MEX', 'CAN', 'RUS', 'WLD']:
        print(f"\n{iso3}:")
        try:
            result = parse_airports(iso3)
            if result:

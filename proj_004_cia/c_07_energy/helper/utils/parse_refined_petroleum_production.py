...
def parse_refined_petroleum_production(pass_data: dict) -> dict:
    """Parse refined petroleum production from CIA Energy section with separated value components."""
    result = {}
        ...
        if 'text' in pass_data:
            ...
            if text and isinstance(text, str):
                ...
                if parsed['value'] is not None:
                    result['refined_production_value'] = parsed['value']
                if parsed['unit']:
                    result['refined_production_unit'] = parsed['unit']
                if parsed['year']:
                    result['refined_production_year'] = parsed['year']
                if parsed['is_estimate']:
                    result['refined_production_is_estimate'] = parsed['is_estimate']

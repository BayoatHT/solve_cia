...
def parse_natural_gas_imports(pass_data: dict) -> dict:
    """Parse natural gas imports from CIA Energy section with separated value components."""
    result = {}
    ...
    try:
        ...
        logging.error(f"Error parsing natural_gas_imports: {e}")
    return result
    try:
        if 'text' in pass_data:
            text = pass_data['text']
            if text and isinstance(text, str):
                result['natural_gas_imports'] = clean_text(text)
    except Exception as e:
        logging.error(f"Error parsing natural_gas_imports: {e}")
    return result
...
if __name__ == "__main__":
    ...
    parsed_data = parse_natural_gas_imports(pass_data)
    print(parsed_data)

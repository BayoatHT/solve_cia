...
def parse_electricity_imports(iso3Code: str, return_original: bool = False)-> dict:
    """Parse electricity imports from CIA Energy section for a given country."""
    result = {}
    try:
    ...
    pass_data = electricity_section.get('imports', {})
    ...
    try:
        ...
        logger.error(f"Error parsing parse_electricity_imports for {iso3Code}: {e}")

    return result

if __name__ == "__main__":
    print("="*60)
    print("Testing parse_electricity_imports")
    print("="*60)
    for iso3 in ['USA', 'ITA', 'DEU', 'BRA', 'FIN', 'WLD']:
        print(f"\n{iso3}:")
        try:
            result = parse_electricity_imports(iso3)
            if result:
                value = result.get('elec_imports_value')
                unit = result.get('elec_imports_unit', '')
                year = result.get('elec_imports_year', 'N/A')
                if value:

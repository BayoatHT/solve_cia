...
def parse_natural_gas(iso3Code: str, return_original: bool = False)-> dict:
    """Parse Natural gas from CIA Energy section for a given country."""
    result = {}
    try:
    ...
    pass_data = energy_section.get('Natural gas', {})
    ...
    try:
        field_mappings = {'production': 'nat_gas_production', 'consumption': 'nat_gas_consumption', 'exports': 'nat_gas_exports', 'imports': 'nat_gas_imports', 'proven reserves': 'nat_gas_reserves'}
        ...
        logger.error(f"Error parsing parse_natural_gas for {iso3Code}: {e}")

    return result

if __name__ == "__main__":
    print("="*60)
    print("Testing parse_natural_gas")
    print("="*60)
    for iso3 in ['USA', 'RUS', 'IRN', 'QAT', 'TKM', 'WLD']:
        print(f"\n{iso3}:")
        try:
            result = parse_natural_gas(iso3)
            if result:
